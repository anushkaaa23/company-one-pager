...
2. **Confidence Feedback Loop**: Implement a multi-agent debate step where an independent "Critic" LLM checks the generated `FactClaim` against the `source_excerpt`. If the Critic finds that the claim is an over-extrapolation of the excerpt, it explicitly downgrades the confidence score below the `0.5` threshold before the final output is compiled.
